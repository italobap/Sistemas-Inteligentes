...
class Map:
    ...
    def get_difficulty(self, coord):
        """ Get the difficulty value associated with a coordinate.
            Returns None if the coordinate is not in the map. """
        entry = self.data.get(coord)
        if entry is None:
            return None
        return entry[0]
    ...
    def get_possible_actions(self, pos):
        """Returns a list of possible actions (dx, dy) from the given position.
        Only directions marked as CLEAR in the map are included."""
        if not self.in_map(pos):
            return []

        actions = []
        action_results = self.get_actions_results(pos)
        ...
        for result, (dx, dy) in zip(action_results, direction_increments):
            if result == VS.CLEAR:
